...
// Read/write operation queue to force these operations to not run concurrently
// Op is a function that takes a callback as its only parameter, which should be called when the operation is finished
FileBufferStream.prototype._awaitOpQueue = function(op) {
	var self = this;
	var opRunner = function() {
		op(function() {
			// Execute next operation if one is enqueued
			if (self.opQueue.length) {
				self.currentOp = self.opQueue.shift();
				self.currentOp();
			} else {
				self.currentOp = null;
			}
	...
	if (self.currentOp) {
		self.opQueue.push(opRunner);
	} else {
		self.currentOp = opRunner;
		opRunner();
	}
}
...
FileBufferStream.prototype._read = function(size) {
	var self = this;
	this._awaitOpQueue(function(opCb) {
		self._fileBufferState.waitingForRead = false;
		self._checkReadMore(size, function(err) {
			if (err) throw err;
			opCb();
		});
	});
...
FileBufferStream.prototype._write = function(chunk, encoding, cb) {
	...
	function doWrite() {
		...
		self._awaitOpQueue(function(opCb) {
...
FileBufferStream.prototype._checkReadMore = function(size, cb) {
	var self = this;
	...
	if (this._fileBufferState.totalWritten > this._fileBufferState.totalRead) {
		...
		if (typeof this._fileBufferState.readFd === 'number') {
			...
			this._readMoreData(size, cb);
		} else {
			// We need to open a new reading FD and read the additional data
			if (this._fileBufferState.openingRead) return cb();
			...
			fs.open(this._fileBufferState.options.filename, 'r', function(err, fd) {
				...
				if (self._fileBufferState.readFd) {
					return cb(new Error('Unexpected parallel open read file'));
				}
				self._fileBufferState.readFd = fd;
				self._readMoreData(size, cb);
			});
		}
...
FileBufferStream.prototype._readMoreData = function(wantedSize, cb) {
	var self = this;
	var size = wantedSize;
	if (!this._fileBufferState.readFd) {
		return cb(new Error('No readFd when in _readMoreData()'));
	}
	if (this._fileBufferState.fileReading) {
		return cb();
	}

	self._fileBufferState.fileReading = true;
	if (!size || size > self._fileBufferState.readBufferSize) {
		size = self._fileBufferState.readBufferSize;
	}
	var readBuffer = new Buffer(size);
	var seekPosition = self._fileBufferState.totalRead;
	fs.read(self._fileBufferState.readFd, readBuffer, 0, size, seekPosition, function(err, bytesRead, readBuffer) {
		...
		if (bytesRead < 1) {
			...
			if (self._fileBufferState.totalRead >= self._fileBufferState.totalWritten && self._fileBufferState.writeEof) {
				...
				cb(new Error('Unexpected EOF reading buffer file'));
			}
		} else {
			self._fileBufferState.totalRead += bytesRead;
			var pushMore = self.push(readBuffer.slice(0, bytesRead));
			if (pushMore) {
				self._readMoreData((!wantedSize || bytesRead >= wantedSize) ? null : (wantedSize - bytesRead), cb);
			} else {
